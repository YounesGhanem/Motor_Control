...
void HAC_Init(EncAlign_Handle_t *pHandle, SpeednTorqCtrl_Handle_t *pSTC, VirtualSpeedSensor_Handle_t *pVSS,
              ENCODER_Handle_t *pENC);

/* Function used to start the encoder alignment procedure */
void HAC_StartAlignment(EncAlign_Handle_t *pHandle);

/* Function used to execute the encoder alignment controller */
bool HAC_Exec(EncAlign_Handle_t *pHandle);

/* It returns true if the encoder has been aligned at least one time */
bool HAC_IsAligned(EncAlign_Handle_t *pHandle);

/* It sets a restart after an encoder alignment */
void HAC_SetRestartState(EncAlign_Handle_t *pHandle, bool restart);

/* Returns true if a restart after an encoder alignment has been requested */
bool HAC_GetRestartState(EncAlign_Handle_t *pHandle);
